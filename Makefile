...
VFILES:=dblib/src/DblibTactics.v\
  dblib/src/DeBruijn.v\
  dblib/src/DemoExplicitSystemF.v\
  dblib/src/DemoImplicitSystemF.v\
  dblib/src/DemoLambda.v\
  dblib/src/DemoValueTerm.v\
  dblib/src/Environments.v\
  Main.v
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/GV/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/GV/$$i; \
	done
...
uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/GV && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "GV" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
